<script lang="ts">
  ...
  const demoUsers = [
    ...
    { name: "Supervisor", pin: "3333", desc: "Refund & Void" },
  ...
  async function submitPin(pinToSubmit: string) {
    ...
    try {
      ...
      showToast(
        `Selamat datang kembali, ${authState.session?.user_name}!`,
        "success",
      );
      goto("/");
    } catch (err: any) {
...
<div
  class="min-h-screen bg-slate-900 flex flex-col justify-center items-center p-4 relative overflow-hidden"
>
  <!-- Glowing Background Orbs -->
  <div
    class="absolute -top-40 -left-40 w-96 h-96 bg-blue-600/20 rounded-full blur-3xl"
  ></div>
  <div
    class="absolute -bottom-40 -right-40 w-96 h-96 bg-indigo-600/20 rounded-full blur-3xl"
  ></div>

  <!-- Main Login Card -->
  <div
    class="w-full max-w-md bg-slate-800/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl p-8 shadow-2xl z-10 transition-all duration-300 {shake
      ? 'animate-shake'
      : ''}"
  >
    <div class="text-center mb-6">
      <div
        class="inline-flex h-14 w-14 rounded-2xl bg-blue-600 items-center justify-center text-white font-black text-2xl shadow-lg shadow-blue-500/20 mb-4"
      >
        P
      </div>
      ...
      <p class="text-slate-400 text-sm mt-1">
        Masukkan PIN keamanan Anda untuk masuk
      </p>
    </div>
    ...
    {#if errorMsg}
      <div
        class="bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs rounded-xl p-3 text-center mb-6 animate-pulse"
      >
        {errorMsg}
      </div>
    {/if}
    ...
    <!-- <div class="border-t border-slate-700/50 pt-6 mt-2">
      <p
        class="text-xs font-semibold text-slate-500 mb-3 text-center uppercase tracking-wider"
      >
        Demo / Testing PINs
      </p>
